...
export async function registerUser(email, displayName, password) {
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      // בדוק שם תצוגה כפול (case insensitive)
      ...
      await sb.from("users").insert({ username: key, display_name: cleanName, password_hash: hash });
  ...
  mem.set(key, { username: key, displayName: cleanName || key.split("@")[0], password: hash, createdAt: new Date().toISOString(), total: 0 });
  return { ok: true, user: { username: key, displayName: cleanName || key.split("@")[0], dailyCount: 0 } };
}
