...
import { getUserLimit } from "../pages/api/credits";
...
export async function registerUser(email, displayName, password) {
  ...
  if (!displayName || displayName.trim().length < 2) return { error: "שם תצוגה חייב להכיל לפחות 2 תווים" };
  ...
  if (isSupabaseConfigured()) {
    try {
      const sb = getSupabase();
      const { data: exEmail } = await sb.from("users").select("username").eq("username", key).single();
      if (exEmail) return { error: "מייל כבר קיים — נסה להתחבר" };
      const { data: exNames } = await sb.from("users").select("username").ilike("display_name", cleanName);
      if (exNames && exNames.length > 0) return { error: "השם הזה תפוס — בחר שם אחר" };
      await sb.from("users").insert({ username: key, display_name: cleanName, password_hash: hash, email_verified: false });
      // שלח קוד אימות
      await sendVerificationCode(key);
      return { ok: true, needsVerification: true, email: key };
    } catch(e) { return { error: "שגיאת רישום: " + e.message }; }
  }
  if (mem.has(key)) return { error: "מייל כבר קיים — נסה להתחבר" };
  mem.set(key, { username: key, displayName: cleanName, password: hash, createdAt: new Date().toISOString(), total: 0 });
  return { ok: true, user: { username: key, displayName: cleanName, dailyCount: 0 } };
}
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      await sb.from("verification_codes").upsert(
        { email: key, code, expires_at: expires, attempts: 0 },
        { onConflict: "email" }
      );
  ...
  try {
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [key],
        subject: "אימות מייל - DR STOCKS 26",
        html: `
          <div dir="rtl" style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;background:#080c14;color:#e2e8f0;padding:32px;border-radius:16px">
            ...
            <p style="color:#94a3b8;margin-bottom:16px">קוד לאימות המייל שלך:</p>
...
export async function verifyEmailCode(email, code) {
  ...
  if (isSupabaseConfigured()) {
    try {
      const sb = getSupabase();
      const { data } = await sb.from("verification_codes").select("code,expires_at,attempts").eq("email", key).single();
      if (!data) return { error: "לא נשלח קוד לכתובת זו" };
      if (new Date(data.expires_at) < new Date()) return { error: "הקוד פג תוקף — הירשם מחדש" };
      if (data.attempts >= 5) return { error: "יותר מדי ניסיונות — הירשם מחדש" };
      if (data.code !== code.trim()) {
        await sb.from("verification_codes").update({ attempts: data.attempts + 1 }).eq("email", key);
        return { error: "קוד שגוי" };
      }
      // אמת את המייל
      await sb.from("users").update({ email_verified: true }).eq("username", key);
      await sb.from("verification_codes").delete().eq("email", key);
      // החזר את פרטי המשתמש
      const { data: user } = await sb.from("users").select("display_name").eq("username", key).single();
      return { ok: true, user: { username: key, displayName: user?.display_name || key, dailyCount: 0 } };
    } catch(e) { return { error: "שגיאת שרת" }; }
  }
  return { error: "Supabase לא מוגדר" };
}
...
export async function loginUser(email, password) {
  ...
  if (isSupabaseConfigured()) {
    try {
      const sb = getSupabase();
      const { data: user } = await sb.from("users").select("username,display_name,password_hash,email_verified").eq("username", key).single();
      ...
      if (!user.email_verified) return { error: "המייל לא אומת — בדוק את תיבת הדואר", needsVerification: true, email: key };
      const limit = await getUserLimit(key);
...
export async function sendResetCode(email) {
  ...
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) { console.log(`[DEV] Reset code for ${key}: ${code}`); return { ok: true, dev: true, code }; }
...
export async function canAnalyze(username) {
  ...
  const limit = await getUserLimit(username);
  ...
  if (isSupabaseConfigured()) {
    try {
      ...
      const [{ count: total }, { count: newUsers }, { data: act }, { data: recent }] = await Promise.all([
        ...
        sb.from("users").select("username,display_name,created_at,total_analyses,daily_limit").order("created_at", { ascending: false }).limit(20),
      ]);
      const cm = {};
